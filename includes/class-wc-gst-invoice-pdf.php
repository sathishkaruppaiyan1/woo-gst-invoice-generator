...
class WC_GST_Invoice_PDF {
    ...
    public function generate_and_download_multiple_pdfs($order_ids) {
        ...
        foreach ($order_ids as $order_id) {
            ...
            // Create a new PDF for this order
            $order_pdf = $this->create_pdf($order_id);
            
            // Get the content of the order PDF
            $order_content = $order_pdf->Output('', 'S');
            
            // Import the page from the order PDF
            $page_count = $pdf->setSourceData($order_content);
...
/**
 * Add order to existing PDF - completely revised to match single invoice formatting
 * 
 * @param TCPDF $pdf The PDF object
